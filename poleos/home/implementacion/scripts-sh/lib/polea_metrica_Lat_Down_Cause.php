    ...
    if (mysqli_query($conn, $sqlTmpTabla)) 
        {
          $sqlInserta = "Insert into tmp_Metrica_".$vRegion." (id_ont, metrica, valor_ant)
                   ...
                   (select Valor_Oid from cat_metricas where id_metrica = 11 and status = 'A'), ObjectId,'.',OntId) Metrica,
                   last_down_cause
                        ...
                        while ($onts = $resOnts->fetch_array())
                           {
                             ...
                             if (!(empty($val_metrica)))
                                ...
                                {
                                   $val_metrica = $onts['valor_ant'];
                                }                             
                              $ActMon = "Update monitor_control_onts ".
                                        " set last_down_cause = '".$val_metrica."', ".
                                        " date_last_down_cause = current_timestamp ". 
                              ...
                              $InsMet = "Insert Into historico_metricas (id_metrica, id_ont, valor, fecha_poleo) values (".
                                        " 11, ".$onts['id_ont'].", '".$val_metrica."', current_timestamp)";
